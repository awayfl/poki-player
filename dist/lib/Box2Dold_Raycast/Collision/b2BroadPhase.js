...
var b2BroadPhase = /** @class */ (function () {
    ...
    b2BroadPhase.prototype.QuerySegment = function (segment, userData, maxCount, sortKey) {
        var uint = function (x) { return x >>> 0; };
        var maxLamda = 1;
        var dx = (segment.p2.x - segment.p1.x) * this.m_quantizationFactor.x;
        var dy = (segment.p2.y - segment.p1.y) * this.m_quantizationFactor.y;
        var sx = dx < -Number.MIN_VALUE ? -1 : (dx > Number.MIN_VALUE ? 1 : 0);
        var sy = dy < -Number.MIN_VALUE ? -1 : (dy > Number.MIN_VALUE ? 1 : 0);
        var p1x = this.m_quantizationFactor.x * (segment.p1.x - this.m_worldAABB.lowerBound.x);
        var p1y = this.m_quantizationFactor.y * (segment.p1.y - this.m_worldAABB.lowerBound.y);
        var startValues = [];
        var startValues2 = [];
        ...
        if (sortKey != null) {
            ...
            for (var i_1 = 0; i_1 < this.m_queryResultCount; i_1++) {
                this.m_querySortKeys[i_1] = sortKey(this.m_proxyPool[this.m_queryResults[i_1]].userData);
            }
            ...
            var i_2 = 0;
            while (i_2 < this.m_queryResultCount - 1) {
                var A = this.m_querySortKeys[i_2];
                var B = this.m_querySortKeys[i_2 + 1];
                if ((A < 0) ? (B >= 0) : (A > B && B >= 0)) {
                    this.m_querySortKeys[i_2 + 1] = A;
                    this.m_querySortKeys[i_2] = B;
                    var tempValue = uint(this.m_queryResults[i_2 + 1]);
                    this.m_queryResults[i_2 + 1] = this.m_queryResults[i_2];
                    this.m_queryResults[i_2] = tempValue;
                    i_2--;
                    if (i_2 == -1)
                        i_2 = 1;
                }
    ...
    b2BroadPhase.prototype.AddProxyResult = function (proxyId, proxy, maxCount, sortKey) {
        var key = sortKey(proxy.userData);
        ...
        while (i < this.m_queryResultCount) {
            var tempKey2 = this.m_querySortKeys[i];
